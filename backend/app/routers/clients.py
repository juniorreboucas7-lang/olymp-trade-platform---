...
def update_client(client_id: int, client: schemas.ClientCreate, db: Session = Depends(database.get_db)):
    ...
    for var, value in client.dict().items():
...
def delete_client(client_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.check_admin_role)):
    db_client = db.query(models.Client).filter(models.Client.id == client_id).first()
    if db_client is None:
        raise HTTPException(status_code=404, detail="Client not found")
    db.delete(db_client)
